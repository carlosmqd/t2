...
<html lang="en">
  ...
  <body>
    ...
    <section id="one" class="content-section-b">
	 ...
	 $miselect= "SELECT `idactividad`, tarea.nombre, `descripcion`, `hora_ini`, `hora_fin`, `tiempo`, `idtipoactividad`, actividad.idestatus,TIME_TO_SEC(timediff(now(),`hora_ini`)) as hora_act FROM `actividad` ,tarea where actividad.idtarea =tarea.idtarea and actividad.idestatus =3 and idusuario = '$idUser' ORDER BY `actividad`.`idactividad` DESC";
  ...
  while($row=mysqli_fetch_array($resultado)) {
   ...
   ?>
	...
	?>
				...
				<td width="15" height="16"><?php echo $descripcion[$i] ?></td>
